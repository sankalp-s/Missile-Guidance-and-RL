...
dH_dU_int = 1000;
h = 0.01; %defining the interval 
t = 0:h:tf; %range from 0 to 5 with inteval of h

ay = 0*ones(length(t),1); %forming a matrix 
...
%   figure(1);  plot(t,states(1,:)); hold on ; grid on
%   figure(2);  plot(t,states(2,:)); hold on ; grid on
%   figure(3);  plot(t,states(3,:)); hold on ; grid on
%   figure(4);  plot(t,states(4,:)); hold on ; grid on
%   figure(5);  plot(t,states(5,:)); hold on ; grid on
%   figure(6);  plot(t,states(6,:)); hold on ; grid on
...
lambda_vf = 0;
lambda_psif =0;
lambda_gammaf = Q*(gammaf-gammafd);
lambda_xf= 0;
lambda_yf= 0;
lambda_zf= 0;
...
%     figure(1);  plot(t,costates(1,:)); hold on ; grid on %changing
%     figure(2);  plot(t,costates(2,:)); hold on ; grid on % zero
%     figure(3);  plot(t,costates(3,:)); hold on ; grid on %changing
%     figure(4);  plot(t,costates(4,:)); hold on ; grid on 
%     figure(5);  plot(t,costates(5,:)); hold on ; grid on
%     figure(6);  plot(t,costates(6,:)); hold on ; grid on


dH_dU_int = 0;
for i=1:length(t)-1 

tau_controlupdate = 1e-3;

lambda_v = costates(1,i); %value is 
lambda_psi = costates(2,i); %value is zero
lambda_gamma = costates(3,i); %value is 
...
dH_dU_ay = ay + conj(lambda_psi)/(v*cos(gamma)) - (2*T*ay*sin((2*m*(ay^2 + az^2)^(1/2))/(S*cl_alpha*rho*v^2))*conj(lambda_v))/(S*cl_alpha*rho*v^2*(ay^2 + az^2)^(1/2));
dH_dU_az = az - conj(lambda_gamma)/v - (2*T*az*sin((2*m*(ay^2 + az^2)^(1/2))/(S*cl_alpha*rho*v^2))*conj(lambda_v))/(S*cl_alpha*rho*v^2*(ay^2 + az^2)^(1/2));

dH_dU(:,i) = [dH_dU_ay , dH_dU_az];

dH_dU_int = dH_dU_int + dH_dU(:,i)'*dH_dU(:,i)*h;

U(:,i) = U(:,i) - tau_controlupdate*(dH_dU(:,i)); %updating the control.
